...
if [ "$(uname -s)" == "Darwin" ]; then
  ...
  if [ "$(whoami)" == "root" ]; then
    TARGET_DIR="/etc/opt/chrome/native-messaging-hosts/blue-focus/"
  else
    TARGET_DIR="$HOME/.config/google-chrome/NativeMessagingHosts/blue-focus/"
  fi
fi

HOST_NAME=br.com.bluefocus.printhost
...
cd "$TARGET_DIR"
HOST_PATH=$TARGET_DIR/printhost.sh
EXTENSION_ID=kbinbmomokkbjjpbhlobfchhpdlifjbh
echo "{
  "\"name"\": "\"br.com.bluefocus.printhost"\",
  "\"description"\": "\"BlueFocus print host for chrome"\",
  "\"path"\": "\"HOST_PATH"\",
  "\"type"\": "\"stdio"\",
  "\"allowed_origins"\": [
...
# Create the executable file to call the python file
echo "python printhost" > "printhost.sh"

# Set execution permissions for the host and the caller executable
chmod +x "$HOST_PATH/printhost*"

# Update host path in the manifest.
ESCAPED_HOST_PATH=${HOST_PATH////\\/}
sed -i -e "s/HOST_PATH/$ESCAPED_HOST_PATH/" "$TARGET_DIR/$HOST_NAME.json"

# Update extension id in the manifest
sed -i -e "s/EXTENSION_ID/$EXTENSION_ID/" "$TARGET_DIR/$HOST_NAME.json"
